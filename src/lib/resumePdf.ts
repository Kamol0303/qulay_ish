...
export async function downloadResumePdf(profile: Profile, templateId?: ResumeTemplateId): Promise<void> {
  ...
  const ensureSpace = (need: number) => {
    ...
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(8);
    doc.text('mexrliqollar.uz rezyume', 8, 16);
  ...
  if (model.isVerified) {
    ...
    doc.text("✓ mexrliqollar.uz tomonidan tasdiqlangan", RIGHT_X, y);
    y += 6;
  }
  y += 2;
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  doc.text(model.isVerified ? '✓ mexrliqollar.uzda tasdiqlangan' : 'mexrliqollar.uz profili', RIGHT_X + 3, y + 6);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7.5);
  ...
  doc.save(`${(model.fullName || 'resume').replace(/\s+/g, '_')}_Mexrli Qo'llar_Resume.pdf`);
}
